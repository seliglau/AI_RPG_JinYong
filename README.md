# 🗡️ 金庸群俠傳 AI RPG

一個以金庸武俠世界為背景的文字 RPG 遊戲。只需上傳單一 Markdown 檔案，即可與任何 AI 開始沉浸式的江湖冒險！<br>
---
如想請作者喝杯咖啡，歡迎[贊助支持](https://ko-fi.com/selig)開發者，你的支持是我繼續創作免費遊戲的最大鼓勵<br>

## 🌟 遊戲特色

* **即插即玩**：無需複雜安裝，下載 `.md` 檔案即可直接交給 AI 運行。
* **高度相容**：支援市面上絕大多數主流 AI 平台。
* **零門檻、低消耗**：不需使用昂貴的高階模型，免費且輕量即可順暢體驗。

---

## 🚀 如何開始遊玩

1. **取得檔案**：按此直接前往下載或查看 [金庸RPG.md](https://github.com/seliglau/AI_RPG_JinYong/blob/main/%E9%87%91%E5%BA%B8RPG.md)。<br>
<img width="791" height="333" alt="image" src="https://github.com/user-attachments/assets/e6023789-8c6f-4c33-b34f-e9d88f797e84" /><br>
2. **上傳 AI**：將檔案直接上傳到你選擇的 AI 聊天視窗中。<br>
<img width="846" height="1046" alt="Screenshot 2026-08-01 160709" src="https://github.com/user-attachments/assets/3aead091-82d6-44ee-abd4-79056736d788" /><br>
3. **踏入江湖**：輸入你的時代，角色名字與出身編號：`元末明初, 金城武, 4`，即可展開武俠人生！<br>
<img width="783" height="972" alt="Screenshot 2026-08-01 160733" src="https://github.com/user-attachments/assets/b2c485a6-f611-47dd-a983-c6fc81badc27" /><br>

---

## 💡 推薦 AI 模型

為了獲得最佳的性價比，推薦使用以下配置：

* **Google Gemini (Flash-Lite / Extended)**：免費且節省 Token，完全不需要動用高階運算模型即可輕鬆暢玩。<br>
<img width="524" height="322" alt="image" src="https://github.com/user-attachments/assets/839bbfd8-b627-4d38-8658-c5796ed876d5" />

---

## 🛠️ 常見問題與疑難排解

* **Q：遇上 AI 模型沒給予【你的行動】選項情況？**  <br>
  A: 如遇上以上情況，請直接輸入：`給我【你的行動】選項`，即可順利繼續遊戲。<br>
<img width="758" height="656" alt="image" src="https://github.com/user-attachments/assets/be814dda-7a19-44ca-b2ea-361ee8d59ea4" />
